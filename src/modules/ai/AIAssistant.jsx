...
function buildDataContext(dataset, columns, types) {
  ...
  // Scale row count to keep total cells under ~3000 so column summaries are never truncated
...
Answer the USER QUESTION above using the DATASET CONTEXT. Be direct and cite specific figures.`
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 3rem)' }}>
        ...
        {messages.length === 0 && !loading ? (
          <div className="flex flex-col items-center justify-center h-full p-8 text-center">
            ...
            <p className="text-xs text-text-secondary mb-6 max-w-sm">
              Click <strong>Auto-Analyse</strong> for an instant overview, or try one of these questions:
            </p>
      ...
      <p className="text-[10px] text-text-muted text-center mt-1.5">Raw data never sent to AI — only computed summaries</p>
    </div>
  )
}
